...
|------|------|------|--------|--------|------|----------------|
| P0 | LINE: `contract_source_q` を KB **より前**に評価 | **今すぐ** | 小 | 中（本番 LINE 回帰） | なし | （Beads 化推奨・未発行） |
| P0 | 重説修正の **reindex + Cloud Run 再デプロイ** | **今すぐ** | 小 | 低 | OpenAI キー | （運用タスク） |
| P1 | `eval_log` にルーティング Tier 別メトリクス追加 | **今すぐ** | 小 | 低 | なし | **AIT-MET-02** |
| P1 | レイテンシ・トークン **1 回計測** | **今すぐ** | 中 | 低 | ログ/Opik | **AIT-MET-01** |
| P2 | `is_important_matters` 時も boost 適用 | **中長期** | 中 | 中 | P0 デプロイ後 | （設計確定後 Beads） |
| P2 | `deal_only` で `master_top_k=0` デフォルト | **中長期** | 中 | **高** | AIT-MET-01/02 | **AIT-TIER-01** |
| P3 | `_route_query` 方針決定 | **中長期** | 小 | 低 | なし | **AIT-RTE-01** |
| P3 | `_route_query` 配線（採用時） | **中長期** | 大 | 高 | AIT-RTE-01 | **AIT-RTE-02** |
| P3 | multi ルーター FP/FN 評価 | **中長期** | 中 | 高 | AIT-RTE-02 | **AIT-RTE-03** |
