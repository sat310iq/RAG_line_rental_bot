"""Tests for answer policy (structured output, no speculation)."""

import pytest
from pydantic import ValidationError
from src.rag_answerer import AnswerSchema, AnswerItem


def test_answer_schema_validation():
    """Test AnswerSchema validation."""
    # Valid schema
    valid_answer = AnswerSchema(
        items=[
            AnswerItem(text="結論です", citation="証拠1"),
            AnswerItem(text="補足です", citation="証拠2"),
        ],
        summary="要約です",
        evidence=["証拠1", "証拠2"],
        next_action="次アクション",
        caveats="注意点",
    )
    assert len(valid_answer.items) == 2
    assert valid_answer.summary == "要約です"
    assert len(valid_answer.evidence) == 2
    
    # Invalid schema (missing fields)
    with pytest.raises(ValidationError):
        AnswerSchema(
            items=[AnswerItem(text="結論のみ", citation="証拠1")]
            # Missing other required fields
        )


def test_answer_schema_no_speculation():
    """Test that answer schema enforces structure (no free-form speculation)."""
    answer = AnswerSchema(
        items=[AnswerItem(text="根拠に基づく結論", citation="doc1")],
        summary="要約",
        evidence=["doc1", "doc2"],
        next_action="具体的な行動",
        caveats="注意事項",
    )
    
    # All required fields must be present
    assert answer.items
    assert answer.summary
    assert answer.evidence
    assert answer.next_action
    assert answer.caveats
    
    # Evidence should be a list
    assert isinstance(answer.evidence, list)
